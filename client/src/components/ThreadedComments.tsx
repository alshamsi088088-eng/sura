...
export function ThreadedComments({ entityId, entityType }: ThreadedCommentsProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(commentsQuery, (snapshot) => {
      const next = snapshot.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<CommentItem, 'id'>) }));
      setComments(next);
    });
  ...
  async function createComment(e: FormEvent) {
    ...
    try {
      const payload = {
        ...
        uid: currentUser.uid,
        author: currentUser.displayName || currentUser.email || 'Reader',
  ...
  async function moderateComment(commentId: string, status: 'visible' | 'hidden') {
    ...
    try {
      await updateDoc(doc(db, 'comments', commentId), { status, moderatedAt: serverTimestamp(), moderatedBy: user?.id || '' });
  ...
  return (
    <section className="mx-auto max-w-[800px] rounded-3xl border border-sura-ivory/10 bg-sura-ink/80 p-6">
      ...
      <form onSubmit={createComment} className="space-y-3">
        ...
        <button
          type="submit"
          disabled={!auth.currentUser || saving || !message.trim()}
          ...
          <article key={comment.id} className="rounded-2xl border border-sura-ivory/10 bg-sura-dark/70 p-4">
            <header className="flex items-center justify-between gap-3">
              <div className="font-inter text-sm text-sura-ivory">
                ...
                <span className="ml-2 text-sura-ivory/60">{new Date((comment.createdAt?.seconds || 0) * 1000).toLocaleString()}</span>
              </div>
              {isModerator ? (
            ...
            {replyTo === comment.id ? (
              <div className="mt-3 space-y-2">
                ...
                <button
                  ...
                  disabled={!auth.currentUser || saving || !replyText.trim()}
