...
export async function homeContent(_req: Request, res: Response) {
  ...
  try {
    ...
    res.json({
      featured: featured.map((item: any) => ({ title: item.title, description: item.excerpt })),
    });
  } catch (error: any) {
  ...
  try {
    ...
    return res.json({
      article: {
        ...
        title: article.title,
        excerpt: article.excerpt,
        coverImage: (article as any).coverImage ?? null,
        content: article.content,
        authorName: (article as any).authorName ?? article.authorName,
